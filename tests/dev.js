(async () => {
  ...
  await client.save('test.json', { hei: 'ja', active: true })
  await client.save('test.number', 3)
  await client.save('test.boolean', true)
  ...
  console.log('== Get one JSON directly ==');
  let blobJSON = await client.get('test.json');
  console.log(blobJSON);
  console.log(blobJSON.data);

  console.log('== Get one number directly ==');
  let blobNumber = await client.get('test.number');
  console.log(blobNumber);

  console.log('== Get one boolean directly ==');
  let blobBoolean = await client.get('test.boolean');
  console.log(blobBoolean);
